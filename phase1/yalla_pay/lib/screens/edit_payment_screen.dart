...
class _UpdatePaymentState extends ConsumerState<UpdatePaymentScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if(!hasLoaded){
      payment = payments.firstWhere((p) => p.id == widget.paymentId);
      
      amount = payment!.amount.toStringAsFixed(1);
      currentOption = payment!.paymentMode;
      
      if (payment!.paymentMode == "Cheque"){
        cheque = cheques.firstWhere((c) => c.chequeNo == payment!.chequeNo);
    ...
    if (isCheque){
      if (!validAmount || !validDate || drawer == '' ||
      (drawer == cheque!.drawer 
      && _dueDateController.text == format.format(cheque!.dueDate) && sameAmount
      && bankName == cheque!.bankName)){
        validUpdate = false;
      } else {
        validUpdate = true;
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('U P D A T E   P A Y M E N T', 
                    ...
                    child: Column(
                      ...
                      children: [
                        ...stylesDetails.headingDetailEntry(
                          ...
                          icon: const Icon(Icons.monetization_on_outlined, color: Colors.grey,)),
                  ...
                  setState(() {
                    ...
                    switch(payment!.paymentMode){
                      case "Cheque":
                        ...
                        cheque!.dueDate = DateTime.parse(_dueDateController.text);
                        cheque!.drawer = drawer;
                        cheque!.bankName = bankName!;
      
                        ref.read(chequeNotifierProvider.notifier).updateChequeDetails(cheque!);
                        ref.read(paymentNotifierProvider.notifier).updatePayment(payment!);
                      default:
                        ref.read(paymentNotifierProvider.notifier).updatePayment(payment!);
                    }
